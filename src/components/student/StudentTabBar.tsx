...
export function StudentTabBar({ assignmentId, onPostClick }: StudentTabBarProps) {
  ...
  const { user } = useAuth();
  const { prefetchAssignments, prefetchFeed, prefetchCourses } = usePrefetchStudentData();
  ...
  // Determine active tab index for sliding indicator
  const getActiveIndex = () => {
    if (isActive('/student/dashboard')) return 0;
    if (isActive('/student/assignments')) return 1;
    if (isActive('/student/courses')) return 3;
    if (isActive('/student/profile')) return 4;
    return -1;
  };
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (isSwipingRef.current && targetTabIndexRef.current !== null) {
        // Map swipe tab index to visual index for the indicator
        const SWIPE_TO_VISUAL = [0, 1, 3, 4]; // Dashboard=0, Assignments=1, Courses=3, Profile=4
    ...
    <>
      ...
      <nav className="fixed bottom-4 left-4 right-4 z-40 px-2 py-2 rounded-2xl native-bottom-nav" style={{ background: 'rgba(255,255,255,0.65)', backdropFilter: 'blur(12px) saturate(150%)', WebkitBackdropFilter: 'blur(12px) saturate(150%)', border: '1px solid rgba(255,255,255,0.5)', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
        <div className="relative flex items-center">
          ...
          <button className="relative flex flex-col items-center w-1/5 py-1 z-10" onClick={() => { animateToTab(0); router.push('/student/dashboard'); }} onTouchStart={prefetchFeed} onMouseEnter={prefetchFeed}>
          ...
          <button className="flex flex-col items-center w-1/5 py-1 z-10" onClick={() => { animateToTab(1); router.push('/student/assignments'); }} onTouchStart={prefetchAssignments} onMouseEnter={prefetchAssignments}>
          ...
          <button className="relative flex flex-col items-center w-1/5 py-1 z-10" onClick={handlePostClick}>
            <svg className={`w-6 h-6 ${inactiveColor}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
            <span className="text-[9px] text-gray-400">Post</span>
          </button>
          <button className="relative flex flex-col items-center w-1/5 py-1 z-10" onClick={() => { animateToTab(3); router.push('/student/courses'); }} onTouchStart={prefetchCourses} onMouseEnter={prefetchCourses}>
          ...
          <button className="relative flex flex-col items-center w-1/5 py-1 z-10" onClick={() => { animateToTab(4); router.push('/student/profile'); }}>
