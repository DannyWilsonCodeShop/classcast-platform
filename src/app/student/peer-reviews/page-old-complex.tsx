  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Load ALL student assignments to get videos across all assignments
        console.log('🔍 Fetching all student assignments for continuous scroll...');
        ...
        if (allAssignmentsResponse.ok) {
          ...
          const assignmentsToLoad = currentAssignmentIndex >= 0
            ? studentAssignments.slice(currentAssignmentIndex) // Current assignment onwards
            : studentAssignments; // All assignments if current not found
          
          console.log('🔍 Loading videos from', assignmentsToLoad.length, 'assignments in order');
          ...
          for (const assignment of assignmentsToLoad) {
            try {
              ...
              if (videosResponse.ok) {
                ...
                console.log(`🎥 Loaded ${videosData.length} videos for assignment:`, assignment.title);
                // Add all videos maintaining submission order
          ...
          console.log('🎥 Total videos loaded across all assignments:', allVideos.length);
          setPeerVideos(allVideos);
        } else {
          // Fallback to single assignment if we can't get all assignments
          console.log('🔍 Falling back to single assignment video load');
