...
const API_NAME = 'ClassCast-Auth-Simple';

async function createSimpleCORSAPI() {
  try {
    console.log('🚀 Creating simple CORS-enabled API Gateway...');
    
    // Create a new API
    const createCommand = new CreateRestApiCommand({
      name: API_NAME,
      description: 'ClassCast Authentication API with Simple CORS',
    ...
    console.log('\n🎉 Simple CORS-enabled API Gateway created successfully!');
    console.log(`\nAPI Endpoints:`);
    ...
    console.error('❌ Error creating simple CORS API Gateway:', error);
    throw error;
  }
}
  ...
  try {
    ...
    try {
      await lambdaClient.send(new AddPermissionCommand({
        ...
        StatementId: `api-gateway-simple-${apiId}-${Date.now()}`,
    ...
    // Now add OPTIONS method for CORS
